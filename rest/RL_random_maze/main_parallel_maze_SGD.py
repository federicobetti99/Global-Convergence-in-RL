...
def run_algs(ProcID):
    ...
    for i in range(Instances_per_Thread):
        ...
        if "SGD" in run_algorithms:
            sim_input = sim_init(num_episodes=num_episodes, gamma=0.99, alpha=0.001, epsilon=0, SGD=1, period=1000)
...
if __name__ == "__main__":
    ...
    with open('SGD_step_maze.pkl', 'wb') as file:
        pickle.dump(SGD_step[0], file)
    with open('SGD_reward_maze.pkl', 'wb') as file:
        # A new file will be created
        pickle.dump(SGD_reward[0], file)
    with open('SGD_step_maze_std.pkl', 'wb') as file:
        pickle.dump(sim_SGD_output_step_std, file)
    with open('SGD_reward_maze_std.pkl', 'wb') as file:
        # A new file will be created

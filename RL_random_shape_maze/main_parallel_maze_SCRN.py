...
def run_algs(ProcID):
    ...
    for i in range(Instances_per_Thread):
        ...
        SCRN_step=[]   
        # Run SCRN
        if "SCRN" in run_algorithms:
            sim_input = sim_init(num_episodes=num_episodes, gamma=0.99, alpha=0.001, epsilon=0, SGD=0, period=10000)
            all_probs, sim_output, temp_goal = discrete_SCRN(sim_input, sim_output)
            SCRN_training_output=temp_goal
            if SCRN_training_output==1:
              SCRN_step = sim_output.step_cache[0]
              SCRN_reward = sim_output.reward_cache[0]
        ...
        #out_thread.append([SGD_step, SCRN_step, REINFORCE_step, SGD_reward, SCRN_reward, REINFORCE_reward, SGD_training_output.tolist(), SCRN_training_output.tolist(), DPG_training_output.tolist()])
        out_thread.append([SCRN_step, SCRN_reward, [SCRN_training_output]])
    return out_thread
...
if __name__ == "__main__":
    ...
    for i in range(len(out)):
        for j in range(len(out[0])):
            #SCRN
            if out[i][j][0]!=[]:
               SCRN_step += out[i][j][0]/np.sum(np.sum(np.array(out).reshape(numthread,Instances_per_Thread,3)[:,:,2]))
            else:
               SCRN_step=SCRN_step
            
            if out[i][j][1]!=[]:
               SCRN_reward += out[i][j][1]/np.sum(np.sum(np.array(out).reshape(numthread,Instances_per_Thread,3)[:,:,2]))
            else:
               SCRN_reward=SCRN_reward
    ...
    sim_SCRN_output_step_std=np.std(np.array(out0),axis=0)    
    if out0==[]:
        sim_SCRN_output_step_std=np.zeros(num_episodes)
     
    sim_SCRN_output_reward_std=np.std(np.array(out1),axis=0)
    if out1==[]:
        sim_SCRN_output_reward_std=np.zeros(num_episodes)


    with open('SCRN_step_maze.pkl', 'wb') as file:   
       # A new file will be created
       pickle.dump(SCRN_step[0], file)
    with open('SCRN_reward_maze.pkl', 'wb') as file:
       # A new file will be created
       pickle.dump(SCRN_reward[0], file)
    with open('SCRN_step_maze_std.pkl', 'wb') as file: 
       # A new file will be created
       pickle.dump(sim_SCRN_output_step_std, file) 
    with open('SCRN_reward_maze_std.pkl', 'wb') as file:   
       # A new file will be created
       pickle.dump(sim_SCRN_output_reward_std, file)
    ...
    sim_out_total.std_alg_reward.append(sim_SCRN_output_step_std)
    ...
    print('SCRN_true_instances:', np.sum(np.sum(np.array(out).reshape(numthread,Instances_per_Thread,3)[:,:,2])))
    x=np.zeros(1)
    ...
    plt.savefig('true_instances_maze_SCRN.pdf')

...
def plot_rewards(
    ...
    for i in range(len(sim_output.reward_cache)):
        ...
        plt.plot([i for i in range(len(mean_reward))], mean_reward, color=alg_colors[i],
                 label=sim_output.name_cache[i])
        plt.fill_between(range(0, len(mean_reward)), mean_reward - 0.2 * var_reward, mean_reward + 0.2 * var_reward,
                         color=alg_colors[i], alpha=0.2)
...
def plot_steps(
    ...
    for i in range(len(sim_output.step_cache)):
        ...
        plt.fill_between(range(0, len(mean_step)), mean_step - 0.2 * var_step, mean_step + 0.2 * var_step,
                         color=alg_colors[i], alpha=0.2)
...
def plot_path(
    ...
    """Plot the latest paths as heatmap"""

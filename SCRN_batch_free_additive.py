...
def f(w):
    return torch.mean((torch.pow(w, 2) + (4.60333333333-b)*torch.pow(torch.sin(w), 2)) + torch.normal(0, 2, size=(1,w.size(dim=1))))
...
def SGD(w_init,eps=None):
    ...
    for i in range(int(Total_oracle_calls)):
        ...
        #print("w:", w)
        #print("grad:",grad)
        delta = (4/(l))*float(1/(i+1))*grad#eta * grad#float(1/(i+1))*grad#eta * grad#float(1/(i+1))*grad#eta * grad#
